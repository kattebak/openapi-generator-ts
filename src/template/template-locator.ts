...
export function createTemplateLocatorChain(
	...
	if (options.customDir) {
		if (options.library) {
			locators.push(
				new CustomLibraryLocator(options.customDir, options.library),
			);
		}
	...
	locators.push(
		new EmbeddedGeneratorLocator(options.embeddedDir, options.generatorName),
	);
