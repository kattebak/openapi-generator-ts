...
describe('HandlebarsAdapter', () => {
  ...
  describe('compile', () => {
    ...
    it('should handle nested properties', () => {
      const engine = createTemplateEngine();
      const template = engine.compile('{{user.name}} is {{user.age}} years old');
    ...
    it('should handle conditionals', () => {
      const engine = createTemplateEngine();
      const template = engine.compile('{{#if isActive}}Active{{else}}Inactive{{/if}}');
  ...
  describe('string lambda helpers', () => {
    it('should support camelcase block helper', () => {
      const engine = createTemplateEngine();
      const template = engine.compile('{{#camelcase}}hello_world{{/camelcase}}');
    ...
    it('should support pascalcase block helper', () => {
      const engine = createTemplateEngine();
      const template = engine.compile('{{#pascalcase}}hello_world{{/pascalcase}}');
      const result = template({});
      assert.strictEqual(result, 'HelloWorld');
    });
  ...
  describe('registerHelper', () => {
    it('should allow registering custom helpers', () => {
      const engine = createTemplateEngine();
      engine.registerHelper('double', (value: number) => String(value * 2));
