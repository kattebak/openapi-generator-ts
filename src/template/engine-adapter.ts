...
import type { TemplatingEngine, CompiledTemplate, TemplateData } from '../core/types.js';
...
export class HandlebarsAdapter implements TemplatingEngine {
  ...
  private registerBuiltInHelpers(): void {
    ...
    for (const [name, fn] of Object.entries(lambdas)) {
      this.handlebars.registerHelper(`lambda.${name}`, fn as Handlebars.HelperDelegate);
    }

    // Helper to check if value is truthy (non-empty array, non-null, etc.)
    this.handlebars.registerHelper('if_not_empty', function (
      this: unknown,
      value: unknown,
    ...
    this.handlebars.registerHelper('unless_empty', function (
      this: unknown,
      value: unknown,
      options: Handlebars.HelperOptions
    ) {
    ...
    this.handlebars.registerHelper('ne', function (a: unknown, b: unknown) {
      return a !== b;
    });

    this.handlebars.registerHelper('gt', function (a: unknown, b: unknown) {
      return (a as number) > (b as number);
    });

    this.handlebars.registerHelper('lt', function (a: unknown, b: unknown) {
      return (a as number) < (b as number);
    });

    this.handlebars.registerHelper('gte', function (a: unknown, b: unknown) {
      return (a as number) >= (b as number);
    });

    this.handlebars.registerHelper('lte', function (a: unknown, b: unknown) {
      return (a as number) <= (b as number);
    });
    ...
    this.handlebars.registerHelper('not', function (value: unknown) {
      return !value;
    });

    // Helper for array operations
    this.handlebars.registerHelper('length', function (value: unknown) {
    ...
    this.handlebars.registerHelper('join', function (
      array: unknown[],
      separator: string
    ) {
    ...
    this.handlebars.registerHelper('-first', function (this: { '@first'?: boolean }) {
      return this['@first'] === true;
    });

    this.handlebars.registerHelper('-last', function (this: { '@last'?: boolean }) {
 ...
 */
export function createTemplateEngine(options?: EngineAdapterOptions): HandlebarsAdapter {
