...
export class SchemaTransformer {
  ...
  transformSchema(name: string, schema: OpenAPIV3.SchemaObject): CodegenModel {
    ...
    // Vendor extensions
    model.vendorExtensions = this.extractVendorExtensions(schema as unknown as Record<string, unknown>);
  ...
  transformPropertySchema(
    ...
    // Vendor extensions
    property.vendorExtensions = this.extractVendorExtensions(schema as unknown as Record<string, unknown>);
   ...
   */
  private getTypeInfo(schema: OpenAPIV3.SchemaObject): Partial<CodegenProperty> {
