...
export class OperationTransformer {
  ...
  transformOperation(
    ...
    const codegenOp = createCodegenOperation(operationId, path, method.toUpperCase());
    ...
    if (security && securitySchemes) {
      for (const secReq of security) {
        for (const [name, scopes] of Object.entries(secReq)) {
          ...
          if (scheme) {
            const authMethod = this.transformSecurityScheme(name, scheme, scopes);
        ...
        variables: server.variables
          ? Object.fromEntries(
              Object.entries(server.variables).map(([key, val]) => [
                key,
                { defaultValue: val.default, description: val.description, enumValues: val.enum },
              ])
            )
          : undefined,
      }));
   ...
   */
  private transformParameter(param: OpenAPIV3.ParameterObject): CodegenParameter {
  ...
  private transformResponse(
    ...
    const codegenResponse = createCodegenResponse(code, response.description ?? '');

    codegenResponse.description = response.description;

    // Response content
    if (response.content) {
      for (const [_mediaType, mediaTypeObj] of Object.entries(response.content)) {
        if (!mediaTypeObj.schema) continue;
    ...
    switch (scheme.type) {
      ...
      case 'oauth2':
        ...
        if (scheme.flows) {
          ...
          if (flow) {
            security.authorizationUrl = (flow as { authorizationUrl?: string }).authorizationUrl;
